...
def get_current_interval():
    now = timezone.now()
    interval_start = now - timedelta(minutes=now.minute % 5,
                                   seconds=now.second,
...
def verifyCodeView(request):
    if request.method == 'POST':
        try:
            ...
            volunteer = Volunteer.objects.get(email=request.user.email)
            coordinators = Coordinator.objects.filter(activity=volunteer.activity)

            if not coordinators:
                coordinators = Coordinator.objects.filter(flagshipEvent=volunteer.activity)

            current_interval = get_current_interval()

            for coordinator in coordinators:
                cache_key = f"coord_code_{coordinator.cname}_{current_interval}"
                code = cache.get(cache_key)
                if code and code == entered_code:
                    return JsonResponse({
                        'success': True,
                        'coord_name': coordinator.cname,
                        'coord_prn': coordinator.prn,
                        'coord_activity': volunteer.activity
                    })

            messages.error(request, 'The code you entered is wrong! Make sure you are asking the code to your Coordinator and not guessing it...')
...
def MarkAttendanceView(request):
    if request.method=='GET':
        ...
        if correct_codes:
            ...
            code_options.append(correct_code)

            while len(code_options) < 4:
                random_code = generate_code()
                if random_code not in code_options:
                    code_options.append(random_code)

            random.shuffle(code_options)

        return render (request, 'mark_attendance.html', {'volunteer': volunteer, 'current': current, 'events': events, 'code_options': code_options})
...
def ChooseCoordinatorView(request):
    ...
    #     coordinators_doing_GP2_activities = Coordinator.objects.filter(activity=volunteer.activity, registered_academic_year=volunteer.registered_academic_year,registered_semester=[1,2])

    #     coordinators_doing_flagship_events = Coordinator.objects.filter(flagshipEvent=volunteer.activity, registered_academic_year=volunteer.registered_academic_year, registered_semester=[1,2])

    #     coordinators = list(chain(coordinators_doing_GP2_activities, coordinators_doing_flagship_events))

    #     return render(request, 'choose-coordinator.html', {
    ...
    if request.method == 'GET':
        ...
        coordinators_doing_GP2_activities = Coordinator.objects.filter(activity=volunteer.activity, registered_academic_year = volunteer.registered_academic_year)

        coordinators_doing_flagship_events = Coordinator.objects.filter(flagshipEvent=volunteer.activity, registered_academic_year = volunteer.registered_academic_year)
