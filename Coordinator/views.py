...
def get_coordinator_code(coordinator_username):
    interval_key = get_current_interval()
    cache_key = f"coord_code_{coordinator_username}_{interval_key}"
...
def verify_coordinator_code(entered_code, activity_name):
    current_interval = get_current_interval()
    coordinators = Coordinator.objects.filter(activity=activity_name)

    for coordinator in coordinators:
        cache_key = f"coord_code_{coordinator.cname}_{current_interval}"
        correct_code = cache.get(cache_key)
        if correct_code and correct_code == entered_code:
            return coordinator
    return None
...
def EventsView(request):
    if request.method == "GET":
        ...
        code = get_coordinator_code(coordinator.cname)

        return render(request, "events.html", {"flagship_events": flagship_events, "activities": activities, "coordinator": coordinator, 'activity_qr': activity_base64, 'flagshipEvent_qr': flagshipevent_base64, "current": current, "code": code})
    else:
...
def ApproveVolunteer(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if attendance:
                volunteer.certificateSent = True
