...
def VolunteerRegistrationView(request):
    ...
    if request.method == 'POST':
        ...
        if len(prn) != 8 or not prn.isdigit():
            messages.error(request, prn + ' does not seem to be a valid PRN. A valid PRN is an eight digit number.')
            return redirect('vreg')

        if len(roll) > 2 or not roll.isdigit():
            messages.error(request, roll + ' does not seem to be a valid student Roll Number. A valid Roll Number is either one or two digit number.')
            return redirect('vreg')
        ...
        if Volunteer.objects.filter(email = emailVal).exists():
            ...
            if v.registered_academic_year == current.AcademicYear and v.registered_semester == current.Semester:
                ...
                elif v.verified == 0 and v.submitted == 0:
                    messages.info(request, 'Hi ' + v.vname + ', you had registered earlier during Academic Year ' + v.registered_academic_year + ' Semester ' + str(v.registered_semester) + 'for the Social Services Course, hence you cannot create an account. If you think this is a mistake, please contact us at vitswd@vit.edu')
            return redirect('vreg')
